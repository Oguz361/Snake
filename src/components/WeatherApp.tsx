...
const WeatherApp: React.FC = () => {
  ...
  return (
    <div className="weather-app comic-style">
      ...
      {weather && (
        <div className="weather-info comic-panel" style={{backgroundImage: getBackgroundColor(weather.main.temp)}}>
          <h2 className="comic-text">{weather.name}</h2>
          <div className="weather-grid">
            <div className="weather-item">
              <p className="comic-text">Temperature</p>
              <p className="comic-text large">{Math.round(weather.main.temp)}°C</p>
            </div>
            <div className="weather-item">
              <p className="comic-text">Feels like</p>
              <p className="comic-text large">{Math.round(weather.main.feels_like)}°C</p>
            </div>
            <div className="weather-item">
            ...
            <div className="weather-item">
              ...
              <p className="comic-text large">{Math.round(weather.wind.speed * 3.6)} km/h</p>
            </div>
            <div className="weather-item">
            ...
            <div className="weather-item">
              ...
              <p className="comic-text">{Math.round(weather.main.temp_max)}°C / {Math.round(weather.main.temp_min)}°C</p>
            </div>
          </div>
        </div>
      )}
      ...
      {forecast && (
        <div className="forecast-container">
          {forecast.list.filter((_, index) => index % 8 === 0).map((day, index) => {
            ...
            return (
              <div key={index} className="forecast-item comic-panel" style={{backgroundImage: getBackgroundColor(day.main.temp)}}>
                ...
                <button className="comic-button" onClick={() => setSelectedDay(index)}>More Info</button>
              </div>
            );
          })}
        </div>
      ...
      {selectedDay !== null && forecast && (
        <div className="day-details comic-panel" style={{backgroundImage: getBackgroundColor(forecast.list[selectedDay * 8].main.temp)}}>
          <h3 className="comic-text">{getDayOfWeek(new Date(forecast.list[selectedDay * 8].dt * 1000))}</h3>
          <div className="weather-grid">
            {forecast.list.slice(selectedDay * 8, (selectedDay + 1) * 8).map((item, index) => (
              <div key={index} className="weather-item">
                <p className="comic-text">{new Date(item.dt * 1000).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                <img
          ...
          <button className="comic-button" onClick={() => setSelectedDay(null)}>Close</button>
        </div>
      )}
    </div>
  );
